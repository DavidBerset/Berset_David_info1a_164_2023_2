        ...
        try:
            with DBconnection() as mc_afficher:
                ...
                if id_film_sel == 0:
                    ...
                    strsql_genres_films_afficher_data += """ HAVING id_boisson= %(value_id_film_selected)s"""
...
"""
    ...
    1) Tous les genres contenus dans la "t_fournisseur".
...
def edit_genre_film_selected():
    if request.method == "GET":
        try:
            ...
            # Récupère la valeur de "id_boisson" du formulaire html "films_genres_afficher.html"
            ...
            # href="{{ url_for('edit_genre_film_selected', id_film_genres_edit_html=row.id_boisson) }}"
            ...
            # Extrait les valeurs contenues dans la table "t_genres", colonne "nom_fournisseur"
...
def update_genre_film_selected():
    if request.method == "POST":
        try:
            ...
            strsql_insert_genre_film = """INSERT INTO t_boisson_acheter_fournisseur (id_boisson_acheter_fournisseur, fk_fournisseur, fk_boisson)
            ...
            strsql_delete_genre_film = """DELETE FROM t_boisson_acheter_fournisseur WHERE fk_fournisseur = %(value_fk_genre)s AND fk_boisson = %(value_fk_film)s"""

            with DBconnection() as mconn_bd:
                # Pour le film sélectionné, parcourir la liste des genres à INSÉRER dans la "t_boisson_acheter_fournisseur".
                ...
                # Pour le film sélectionné, parcourir la liste des genres à EFFACER dans la "t_boisson_acheter_fournisseur".
                # Si la liste est vide, la boucle n'est pas parcourue.
...
def genres_films_afficher_data(valeur_id_film_selected_dict):
    ...
    try:
        ...
        strsql_genres_films_non_attribues = """SELECT id_fournisseur, nom_fournisseur FROM t_fournisseur WHERE id_fournisseur not in(SELECT id_fournisseur as idGenresFilms FROM t_boisson_acheter_fournisseur
                                                    INNER JOIN t_boisson ON t_boisson.id_boisson = t_boisson_acheter_fournisseur.fk_boisson
                                                    INNER JOIN t_fournisseur ON t_fournisseur.id_fournisseur = t_boisson_acheter_fournisseur.fk_fournisseur
                                                    WHERE id_boisson = %(value_id_film_selected)s)"""

        strsql_genres_films_attribues = """SELECT id_boisson, id_fournisseur, nom_fournisseur FROM t_boisson_acheter_fournisseur
                                            INNER JOIN t_boisson ON t_boisson.id_boisson = t_boisson_acheter_fournisseur.fk_boisson
                                            INNER JOIN t_fournisseur ON t_fournisseur.id_fournisseur = t_boisson_acheter_fournisseur.fk_fournisseur
                                            WHERE id_boisson = %(value_id_film_selected)s"""
